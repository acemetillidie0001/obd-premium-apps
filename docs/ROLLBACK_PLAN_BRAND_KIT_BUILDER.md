...
**Note:** BrandProfile migration is **additive only** (no data loss risk). This step is usually **NOT necessary** and can be skipped. Only rollback if:
...
**Data Loss Warning:** This permanently deletes all saved brand profiles. Only proceed if absolutely necessary.
...
- **BrandProfile Migration Safety:** The BrandProfile migration is additive (creates new table, adds relation). It does NOT modify existing tables or data. Safe to keep even if code is rolled back.

- **Environment Variables:** Rolling back code does NOT revert environment variables. Remove manually if needed.

- **Cold Start Impact:** In-memory rate limiting resets on serverless cold starts. This is expected behavior, not a bug.

- **RequestId Tracing:** All errors include requestId. Use these to trace issues in logs before rolling back.
